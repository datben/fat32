...
{
    std::memcpy(jump_boot, buffer, 3);
    std::memcpy(oem_name, buffer + 3, 8);
    std::memcpy(&bytes_per_sector, buffer + 11, 2);
    std::memcpy(&sectors_per_cluster, buffer + 13, 1);
    std::memcpy(&reserved_sector_count, buffer + 14, 2);
    std::memcpy(&table_count, buffer + 16, 1);
    std::memcpy(&root_entry_count, buffer + 17, 2);
    std::memcpy(&total_sectors_16, buffer + 19, 2);
    std::memcpy(&media_type, buffer + 21, 1);
    std::memcpy(&table_size_16, buffer + 22, 2);
    std::memcpy(&sectors_per_track, buffer + 24, 2);
    std::memcpy(&head_side_count, buffer + 26, 2);
    std::memcpy(&hidden_sector_count, buffer + 28, 4);
    std::memcpy(&total_sectors_32, buffer + 32, 4);
    std::memcpy(&table_size_32, buffer + 36, 4);
    std::memcpy(&extended_flags, buffer + 40, 2);
    std::memcpy(&fat_version, buffer + 42, 2);
    std::memcpy(&root_cluster, buffer + 44, 4);
    std::memcpy(&fat_info, buffer + 48, 2);
    std::memcpy(&backup_boot_sector, buffer + 50, 2);
    std::memcpy(reserved_0, buffer + 52, 12);
    std::memcpy(&drive_number, buffer + 64, 1);
    std::memcpy(&reserved_1, buffer + 65, 1);
    std::memcpy(&boot_signature, buffer + 66, 1);
    std::memcpy(&volume_id, buffer + 67, 4);
    std::memcpy(volume_label, buffer + 71, 11);
    std::memcpy(fs_type, buffer + 82, 8);
};
